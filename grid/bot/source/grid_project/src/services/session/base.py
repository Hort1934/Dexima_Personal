...
class BaseSession(abc.ABC):
    ...
    def check_response(  # noqa: C901
        ...
        **kwargs: Any,
        ...
        if HTTPStatus.OK <= status_code <= HTTPStatus.IM_USED:
            return Response(status_code=status_code, result=data, **kwargs)
    ...
    def prepare_value(self, value: Any, _dumps_json: bool = True) -> Any:  # noqa: C901
        if value is None:
            return None
        ...
        if isinstance(value, dict):
            value = {
                ...
                if (prepared_item := self.prepare_value(item, _dumps_json=False))
                is not None
            }
            if _dumps_json:
