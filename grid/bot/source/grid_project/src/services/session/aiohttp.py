...
from source.grid_project.src.services.session import errors as err
from source.grid_project.src.services.session.base import BaseSession, _RequestMethod
from source.grid_project.src.services.session.response import Response, ResultType
...
class AiohttpSession(BaseSession):
    ...
    def build_data(
        self, method: Union[BaseModel, Dict[str, Any]]
    ) -> Union[FormData, bytes, str]:
        form = FormData()
        if isinstance(method, BaseModel):
            method = method.model_dump(
                warnings=False, exclude_none=True, exclude_unset=True
            )
            ...
            raise err.NotValidMethodError(
                f"Expected BaseModel instance, dict, str or bytes, not {type(method)}"
            )
