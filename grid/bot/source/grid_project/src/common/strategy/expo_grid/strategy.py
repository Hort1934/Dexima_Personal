...
class ExpoGridStrategy(BaseStrategy[ExpoGridStrategySettings]):
    ...
    async def execute(self) -> None:
        ...
        if self.grid.empty:
            ...
            for line, value in self.grid.values.items():
                try:
                    self.executor.amount_to_precision(value, self.settings.symbol)
                except Exception as e:
                    if "must be greater than minimum amount precision" in (
                        msg := str(e)
                    ):
                        min_qty = float(msg.split[-1])
                        self.grid.values[line] = min_qty
                    else:
                        raise e
    ...
    async def _process_out_of_range(
        ...
        await self.grid.reset(save=not self.executor.is_backtester, bot_id=self.bot_id)
    ...
    async def _process_no_positions(self) -> None:
        ...
        o1 = await self.executor.create_order(
            symbol=self.settings.symbol,
            type="limit",
            side=side,
            ...
            params={
                ...
                "triggerDirection": 2,
                "base_price": lower_grid,
            },
        )
        o2 = await self.executor.create_order(
            symbol=self.settings.symbol,
            type="limit",
            side=side,
            ...
            params={
                ...
                "triggerDirection": 1,
                "base_price": upper_grid,
            },
        )

        o1_ok = o1.get("info", {}).get("orderId")
        o2_ok = o2.get("info", {}).get("orderId")

        if o1_ok and o2_ok:
            logger.info(f"Placed 2 orders on {lower_grid} and {upper_grid} levels")
        else:
            logger.info("Error while placing orders")
            if o1_ok:
                logger.info(f"Order {o1_ok} has been cancelled due to error")
                await self.executor.cancel_order(
                    id=o1_ok,
                    symbol=self.settings.symbol,
                )
            if o2_ok:
                logger.info(f"Order {o2_ok} has been cancelled due to error")
                await self.executor.cancel_order(
                    id=o2_ok,
                    symbol=self.settings.symbol,
                )
